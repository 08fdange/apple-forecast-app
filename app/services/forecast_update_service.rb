class ForecastUpdateService
  ...
  def self.update_forecast(zip_code)
    redis_key = "forecast:#{zip_code}"
    forecast_data, cached = fetch_from_cache(redis_key)

    unless cached
      forecast_data, cached = fetch_from_database(zip_code)
      unless cached
        forecast_data, cached = fetch_and_update(zip_code, redis_key)
      end
    end
  ...
  def self.fetch_from_database(zip_code)
    ...
    if forecast && !forecast.expired?
      ...
      $redis.set("forecast:#{zip_code}", forecast.forecast_data.to_json, ex: EXPIRATION_IN_SECONDS)
  ...
  def self.fetch_and_update(zip_code, redis_key)
    fetched_data = WeatherService.fetch_weather(zip_code)
